...
const generateAbout = aboutText => {  //instuct the function to render this section elemnet with content only if there's content to render in the first place
  if (!aboutText) {
    return '';
  }
...
const generateProjects = projectsArr => {
  ...
  const featuredProjectHtmlArr = featuredProjects.map(({ name, description, languages, link }) => { // use the .join to turn the lanuages used into a combined string
    return `  
